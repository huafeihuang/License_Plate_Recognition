...
class GenPlateScene:
    ...
    def gen_batch(self, batchSize, outputPath):
        ...
        for i in range(batchSize):
            ...
            count+=1
            print(count)
            print(outputPath + "/" + str(count).zfill(2) + ".jpg")
            cv2.imwrite(outputPath + "/" + str(count).zfill(2) + ".jpg", img)
            with open(outputPath + "/" + str(count).zfill(2) + ".txt", 'w', encoding='utf-8') as obj:
...
def main():
    G = GenPlateScene("./font/platech.ttf", './font/platechar.ttf', "./NoPlates")
    G.gen_batch(100000, './dataSet')
